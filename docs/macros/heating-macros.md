...
- Температура с которой начинается уменьшение мощности прибора вплоть до правой стороны гистерезиса. По умолчанию это целевая температура.
  ...
  - Например, требуемая температура 28, гистерезис слева 26, гистерезис справа 29, температура с которой начинается уменьшение мощности 27.5, на промежутке от 26 до 27.5 запрос тепла в процентном отношении 100%, при достижении 27.5, начинается пропорциональное снижение запроса тепла, а при достижении 29 градусов, запрос тепла 0% и петля отключает запрос тепла.
...
- Запрос на температуру подачи.
- Текущая температура подачи.
- Текущая температура обратки.
- Такты в секундах между вычислениями следующего положения смесителя.
- Коэффициент пропорционального регулирования.

#### Исходящих аргументов:

- Положение смесительного крана.

#### Логика работы:

Смеситель принимает запрос на температуру подающей линии, а так же температуру подачи и обратки после смесителя.

Когда подача превышает значение запроса тепла, нужно повернуть кран смесителя таким образом, чтобы температура подачи стала близка к запрошенной.

Процесс вычисления следующего положения смесителя происходит бесконечно такт за тактом, а время между тактами задается в настройках.

В каждом такте происходит вычисление разницы между запрошенной температурой подачи, это значение называется ошибкой.

Если ошибка отрицательная, то смеситель открыт на прямую.
Если ошибка положительная, то смеситель должен начать менять положение в сторону подмеса обратной линии на какой-то процент от 0 до 100%.

Значение ошибки умножается на коэффициент пропорциональности, который по сути ускоряет или замедляет изменение положения смесителя.

Как теперь понять на сколько крутить относительно ошибки ????

Если крутить на величину ошибки умноженную на коэффициент то можно перекрутить кран слишком быстро и потом температура упадет и придется крутить его обратно и таким образом, кран будет крутиться туда сюда, и система будет не стабильна.

Интегральное регулирование, это сложение
