...
- Тип нагреваемой среды
  ...
  - Если выбрано устройство, для него может быть реализована специальная логика по мимо общей логики.

- Минимальная значение запроса температуры подающей линии
  - Для теплого пола 30
  - Для радиатора 30
  - Для дорожки на улице 55
  - Для бойлера 60
  - Ниже это температуры нельзя будет запросить
  - Безусловный нижний предел это 30
...
- Наличие расхода
  ...
  - Если все реле **ВЫКлючены**, то все сервоприводы, переводятся в нормальное положение
...
Если достигнута температура после которой температура пропорционально снижается, то запрос тепла уменьшается в процентах пропорционально росту температуры до правого гистерезиса.
...
- "Термостат включен" или запрошено тепло, логическое значение, которое принимает два состояния, правда или лож, что интерпретируется как греть или не греть
...
- Отправляет сигнал на твердотельное реле
  - Тип контрола switch
